import { formatDate } from "../utils/authUtils";

/**
 * Posts can be added here.
 * You can add default posts of your wish with different attributes
 * */

export const posts = [
  {
    _id: "xhzTkUi2Nt",
    content: "Developers are the architects of the digital world!!",
    mediaURL: "https://cdn.wallpapersafari.com/91/28/AKZNHx.jpg",
    likes: {
      likeCount: 5,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [
      {
        _id: "okzxcf",
        username: "janedoe",
        text: "I agree!! Feels like we should go for outing. What say?",
        votes: {
          upvotedBy: [],
          downvotedBy: [],
        },
      },
    ],
    username: "abhishek",
    createdAt: "2023-06-10T10:55:06+05:30",
    updatedAt: formatDate(),
  },
  {
    _id: "siFFxfYI1s",
    content: "Success in trading comes from the discipline to consistently follow your strategy.",
    mediaURL:
      "https://media.gettyimages.com/id/1317587887/photo/trading-charts-on-a-display.jpg?s=612x612&w=gi&k=20&c=-1OWpoM21zE_Rn0c-MZpnyZTJt-C6Y489wefF1jh_Vw=",
    likes: {
      likeCount: 0,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [],
    username: "carlsmith",
    createdAt: "2023-06-21T10:55:06+05:30",
    updatedAt: formatDate(),
  },
  {
    _id: "3XHvLP1fg",
    content: "Frontend development is about finding the perfect balance between aesthetics and functionality ",
    mediaURL:
      "https://pbs.twimg.com/profile_images/1648635693932122115/W12SFpGW_400x400.jpg",
    likes: {
      likeCount: 1,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [
      {
        _id: "6nImWvImxo",
        username: "johndoe",
        text: "That's mouth watering! Could you make one for my Birthday as well?",
        votes: {
          upvotedBy: [],
          downvotedBy: [],
        },
      },
    ],
    username: "janedoe",
    createdAt: "2023-06-10T10:55:06+05:30",
    updatedAt: formatDate(),
  },
  // {
  //   _id: "MTYtVhecCj",
  //   content:
  //     "Every sunset brings the promise of a new dawn.",
  //   mediaURL: "https://t3.ftcdn.net/jpg/04/36/67/54/360_F_436675446_jGWzkVDah3b6ONZxhhN13s6I4iFnjLGJ.jpg",
  //   likes: {
  //     likeCount: 5,
  //     likedBy: [],
  //     dislikedBy: [],
  //   },
  //   comments: [],
  //   username: "adarshbalika",
  //   createdAt: "2023-06-06T10:55:06+05:30",
  //   updatedAt: formatDate(),
  // },
  // {
  //   _id: "3XHvLP9kC",
  //   content: "Amidst the darkness, let the light of unity and harmony illuminate Ayodhya!",
  //   mediaURL:
  //     "https://c4.wallpaperflare.com/wallpaper/61/50/360/night-lights-india-river-barn-ayodhya-hd-wallpaper-preview.jpg",
  //   likes: {
  //     likeCount: 2,
  //     likedBy: [],
  //     dislikedBy: [],
  //   },
  //   comments: [
  //     {
  //       _id: "EO7iOPN9n8",
  //       username: "carlsmith",
  //       text: "Happy Birthday! Enjoy your Day!!",
  //       votes: {
  //         upvotedBy: [],
  //         downvotedBy: [],
  //       },
  //     },
  //     {
  //       _id: "QK52wf6HI1",
  //       username: "janedoe",
  //       text: "Wow! Looks Amazzing! Happy Birthday!",
  //       votes: {
  //         upvotedBy: [],
  //         downvotedBy: [],
  //       },
  //     },
  //   ],
  //   username: "adarshbalika",
  //   createdAt: "2023-06-15T10:55:06+05:30",
  //   updatedAt: formatDate(),
  // },
  // {
  //   _id: "tHaThLyFQh",
  //   content:
  //     "Your education is a dress rehearsal for a life that is yours to lead.",
  //   mediaURL: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ8rpohBoKbOEaXarE_0xDIWdVAni8DxR4onw&usqp=CAU",
  //   likes: {
  //     likeCount: 2,
  //     likedBy: [],
  //     dislikedBy: [],
  //   },
  //   comments: [],
  //   username: "adarshbalika",
  //   createdAt: "2023-06-06T10:55:06+05:30",
  //   updatedAt: formatDate(),
  // },
  // {
  //   _id: "G-Gi3lSZP9",
  //   content: "A true friend is someone who is there for you when they would rather be anywhere else.",
  //   mediaURL: "https://e1.pxfuel.com/desktop-wallpaper/835/168/desktop-wallpaper-4-friendship-funny-quotes-on-college-life-college-friends.jpg",
  //   likes: {
  //     likeCount: 2,
  //     likedBy: [],
  //     dislikedBy: [],
  //   },
  //   comments: [],
  //   username: "adarshbalika",
  //   createdAt: "2023-06-06T10:55:06+05:30",
  //   updatedAt: formatDate(),
  // },
  // {
  //   _id: "GeMUAdi9mh",
  //   content:
  //     "Embrace the challenges of today and let your relentless effort pave the way for a brighter tomorrow",
  //   mediaURL: "https://images7.alphacoders.com/418/418327.jpg",
  //   likes: {
  //     likeCount: 2,
  //     likedBy: [],
  //     dislikedBy: [],
  //   },
  //   comments: [],
  //   username: "adarshbalika",
  //   createdAt: "2023-06-06T12:55:06+05:30",
  //   updatedAt: formatDate(),
  // },
  // {
  //   _id: "IyUlYXTrzZ",
  //   content: "Who's up for Pineapple Pastry?",
  //   mediaURL:
  //     "https://res.cloudinary.com/dtrjdcrme/image/upload/v1647014828/ecommerce/pineapplemuffin1.webp",
  //   likes: {
  //     likeCount: 10,
  //     likedBy: [],
  //     dislikedBy: [],
  //   },
  //   comments: [
  //     {
  //       _id: "6nIffvImxo",
  //       username: "sophiajones",
  //       text: "Oh! That looks Delicious!",
  //       votes: {
  //         upvotedBy: [],
  //         downvotedBy: [],
  //       },
  //     },
  //   ],
  //   username: "abhisheksingh",
  //   createdAt: "2023-06-10T10:55:06+05:30",
  //   updatedAt: formatDate(),
  // },
  {
    _id: "Ie92xOSVeU",
    content:
      "For build your logic and humour so strong then you do code",
    mediaURL: "https://www.wikihow.com/images/thumb/e/ed/Code-Step-5-Version-4.jpg/v4-460px-Code-Step-5-Version-4.jpg.webp",
    likes: {
      likeCount: 1,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [
      {
        _id: "6fgd534s",
        username: "janedoe",
        text: "Yes! I am also learning from there!",
        votes: {
          upvotedBy: [],
          downvotedBy: [],
        },
      },
    ],
    username: "alexmaxwell",
    createdAt: "2023-06-12T10:55:06+05:30",
    updatedAt: formatDate(),
  },
  {
    _id: "xhzTkUyfNt",
    content: "Believe in yourself! Rest all will fall in place.",
    mediaURL: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS1Z52HTWpwiLQIqfqHtgFhEEucXlmlVhTkuQ&usqp=CAU",
    likes: {
      likeCount: 2,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [
      {
        _id: "unImWvIzbf",
        username: "abhishek",
        text: "So True!!",
        votes: {
          upvotedBy: [],
          downvotedBy: [],
        },
      },
    ],
    username: "sophiajones",
    createdAt: "2023-06-15T10:55:06+05:30",
    updatedAt: formatDate(),
  },
  {
    _id: "CCmJpDnnQQ",
    content:
      "You can get everything in life you want if you will just help enough other people get what they want.",
    mediaURL: "https://images.alphacoders.com/738/738123.jpg",
    likes: {
      likeCount: 5,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [],
    username: "abhishek",
    createdAt: "2023-06-10T10:55:06+05:30",
    updatedAt: formatDate(),
  },

  {
    _id: "Z_TZT_3EAw",
    content:
      "If you believe something needs to exist, if it's something you want to use yourself, don't let anyone ever stop you from doing it.",
    mediaURL: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQIrqjCFJ2MVpQQdQZF3luiuBRolELYpOsHmQ&usqp=CAU",
    likes: {
      likeCount: 5,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [],
    username: "johndoe",
    createdAt: "2023-06-12T10:55:06+05:30",
    updatedAt: formatDate(),
  },

  {
    _id: "e-knMEsoLq",
    content: "More is lost by indecision than wrong decision.",
    mediaURL: "https://www.shutterstock.com/image-illustration/decision-making-word-cloud-concept-260nw-261312809.jpg",
    likes: {
      likeCount: 5,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [],
    username: "alexmaxwell",
    createdAt: "2023-06-26T10:55:06+05:30",
    updatedAt: formatDate(),
  },

  {
    _id: "VY14RsXC7G",
    content:
      "The reason we struggle with insecurity is because we compare our behind-the-scenes with everyone else’s highlight reel.",
    mediaURL: "https://images.unsplash.com/photo-1545241201-fee9df605ca8?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8c3RydWdnbGV8ZW58MHx8MHx8fDA%3D&w=1000&q=80",
    likes: {
      likeCount: 7,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [],
    username: "carlsmith",
    createdAt: "2023-06-10T10:55:06+05:30",
    updatedAt: formatDate(),
  },

  {
    _id: "l9pedEMjZS",
    content: "If you don’t risk anything, you risk even more.",
    mediaURL: "https://media.istockphoto.com/id/1155623694/photo/symbol-for-reducing-a-risk-cubes-form-the-word-risk-while-a-hand-turns-a-cube-and-changes-the.jpg?s=612x612&w=0&k=20&c=vmLwQ6opnMuM3YKfOQuk0TLO07qqByfYsUoJgfLCRrc=",
    likes: {
      likeCount: 6,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [],
    username: "sophiajones",
    createdAt: "2023-06-10T10:55:06+05:30",
    updatedAt: formatDate(),
  },

  {
    _id: "lbW4dlCpNC",
    content: "If it makes you nervous, you’re doing it right.",
    mediaURL: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSttR3NxBpkPiBrFU3kyiVcxKTuEhgwKgz-HQ&usqp=CAU",
    likes: {
      likeCount: 12,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [],
    username: "abhishek",
    createdAt: "2023-06-10T10:55:06+05:30",
    updatedAt: formatDate(),
  },

  {
    _id: "k-Vwd2d7Vt",
    content:
      "I learned a long time ago that there is something worse than missing the goal, and that’s not pulling the trigger.",
    mediaURL: "https://thumbs.dreamstime.com/b/stand-out-crowd-different-creative-idea-concepts-longest-light-ladder-glowing-aiming-high-to-goal-target-other-116711139.jpg",
    likes: {
      likeCount: 9,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [],
    username: "carlsmith",
    createdAt: "2023-06-14T10:55:06+05:30",
    updatedAt: formatDate(),
  },

  {
    _id: "R2lAuXvK7V",
    content:
      "Success is stumbling from failure to failure with no loss of enthusiasm. Love this quote by Winston Churchill.",
    mediaURL: "https://images.unsplash.com/photo-1497561813398-8fcc7a37b567?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTF8fHN1Y2Nlc3N8ZW58MHx8MHx8fDA%3D&w=1000&q=80",
    likes: {
      likeCount: 12,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [],
    username: "janedoe",
    createdAt: "2023-06-10T10:55:06+05:30",
    updatedAt: formatDate(),
  },

  {
    _id: "G5xd30tMCR",
    content:
      "Optimism is the faith that leads to achievement. Nothing can be done without hope and confidence.",
    mediaURL: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSJ1ePoOGUhpObfeHbpaO_htM8aOvT_h8ZODA&usqp=CAU",
    likes: {
      likeCount: 8,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [],
    username: "alexmaxwell",
    createdAt: "2023-06-14T10:55:06+05:30",
    updatedAt: formatDate(),
  },

  {
    _id: "lF8cnfPAe9",
    content:
      "Life is like riding a bicycle. To keep your balance you must keep moving.",
    mediaURL: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQJHfv7qmfS8O27-R367WIGg5_FJOftmmf-tA&usqp=CAU",
    likes: {
      likeCount: 5,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [],
    username: "abhishek",
    createdAt: "2023-06-10T10:55:06+05:30",
    updatedAt: formatDate(),
  },

  {
    _id: "stfTkUi2Nt",
    content:
      "“Do not wait for the perfect time and place to enter, for you are already onstage.",
    mediaURL: "https://images.unsplash.com/photo-1541339907198-e08756dedf3f?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8M3x8Y29sbGVnZXxlbnwwfHwwfHx8MA%3D%3D&w=1000&q=80",
    likes: {
      likeCount: 25,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [],
    username: "sophiajones",
    createdAt: "2023-06-10T10:55:06+05:30",
    updatedAt: formatDate(),
  },

  {
    _id: "quiTkUi2Nt",
    content: "Success is not final, failure is not fatal: It is the courage to continue that counts.",
    mediaURL: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTGUZQoAF-qFSu5Jgtgdq0r56u4kIjGhJPE-w&usqp=CAU",
    likes: {
      likeCount: 8,
      likedBy: [],
      dislikedBy: [],
    },
    comments: [],
    username: "janedoe",
    createdAt: "2023-06-10T10:55:06+05:30",
    updatedAt: formatDate(),
  },
];